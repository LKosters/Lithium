...
// SQLite is the source of truth; reads return detached values, never a mutable cache.
const { getDatabase } = require("./database");
function loadConfig() { return getDatabase().get('config', { recentDirs: [] }); }
function saveConfig(config) { getDatabase().set('config', config); }
...
// ── Session persistence ──────────────────────────────
function loadAllSessions() { return getDatabase().sessions(); }
function saveSession(session) {
  if (session && isValidSessionId(session.id)) getDatabase().saveSession(session);
}
function deleteSession(id) { if (isValidSessionId(id)) getDatabase().deleteSession(id); }
function saveLayoutToDisk(layout) { getDatabase().set('layout', layout); }
function loadLayoutFromDisk() { return getDatabase().get('layout'); }
