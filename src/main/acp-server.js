...
function startACPServer() {
  ...
  console.log("[acp-server] Starting codex-acp...");
  ...
  acpProcess.stderr.on("data", (data) => {
    console.error("[acp-server:stderr]", data.toString().trim());
  });
...
async function doStartup() {
  try {
    ...
    // Still mark as ready so the user can see it's running
    // Auth might have failed but process is alive
    acpReady = true;
  }
}
