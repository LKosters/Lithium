...
- **Renderer process** (`src/index.html` + `src/renderer.js` + `src/renderer/*.js`) —
  the Chromium window. Owns the DOM, xterm.js terminals, the split layout tree, and
...
| `src/main/dev-server.js` | Per-project start-command runner + URL detection |
...
1. **`require("./src/main/shell-env").fixPath()`** (`main.js:8`) — **runs before anything
   ...
   `claude` and `npx` fail at spawn with `env: node: No such file or
...
7. **`./src/main/browser-bridge` and `./src/main/updater`**, then
   `registerUpdaterHandlers()` is called explicitly. These modules expose a
   `register*()` function rather than registering on require — so their handlers
   attach at a controlled moment.
...
1. **Create the hub, then require feature modules** (`renderer.js:4-18`). `app` (from
   ...
   `browser`, `music`, `settings`, `git`, `directory`).
2. **Wire functions onto `app`** (`renderer.js:21-59`). This is the crux of the module
   ...
   (`app.createTerminal`, `app.openTab`, `app.refreshLayout`, …)
...
(`createTerminal` + `pty:spawn` with `resume: true`), renders the
...
`Map<sessionId, { term, fitAddon, paneEl, alive }>` that is the renderer's registry of
live panes — and `collapsedDirs` (a `Set` for sidebar UI).
...
| `project:create` | invoke/handle | `{ framework, name, projectsDir }` → `{ ok, dir }` / `{ ok:false, error }` (supports `nextjs`, `nodejs`) |
| `project:get-settings` | invoke/handle | `dir` → `{ startCommands: string[] }` (from `<dir>/.lithium/settings.json`) |
| `project:set-settings` | invoke/handle | `{ dir, settings }` → `{ ok }` |
...
| Channel | Kind | Payload / return |
| --- | --- | --- |
| `devserver:start-commands` | invoke/handle | `{ cwd }` → `string[]` of resolved start commands |
| `devserver:start` | invoke/handle | `{ cwd }` → `{ ok, commands }` / `{ ok: false, error }` |
| `devserver:stop` | invoke/handle | → `{ ok }` |
| `devserver:url` | main→renderer | detected `http://localhost:PORT` |
| `devserver:stopped` | main→renderer | (no payload) once the last process exits |
...
- **Guard `webContents` before pushing.** Every main→renderer `send` first checks
  `!senderWebContents.isDestroyed()` (e.g. `pty.js:47,62,70`) — the
...
- macOS-specific: `media.js` (AppleScript now-playing) and the `Settings…` menu item are
  ...
  `detached`.
- The browser bridge still starts at app-ready, but since ACP chat was removed nothing
  registers the `browser` MCP server that consumed it. See
...
- **2026-07-22** — Removed ACP chat support: deleted `agents.js`,
  `provider-registry.js`, `acp-server*.js`, `providers/`, `renderer/chat.js`, and
  `styles/chat.css`, along with every `agent:*` channel and `dialog:pick-images`.
  Every session is now a Claude Code CLI PTY. Added per-project settings stored in
  `<project>/.lithium/settings.json` (`project:get-settings` / `project:set-settings`)
  and reworked the dev-server channels around them.
